...
#include <QDebug>
#include <string>
#include "test_detectmotion.h"
using namespace cv;
using namespace std;

Cam::Cam(QWidget * parent) : QWidget(parent)
{
    int frameWidth=620;
    int frameHeight=440;
    int subImageWidth=620;
    int subImageHeight=200;
   ...
   while (waitKey(5)<0)
    {
        ...
        if(workingCenter.x+vect.x<305){
            choix="gauche";


        }
        if(workingCenter.x+vect.x>315){
            choix="droite";

        }

        //cout<<choix<<"vitesse : "<<workingCenter.x+vect.x-310<<endl;
